...
export const myRecordings = query({
  ...
  handler: async (ctx) => {
    ...
    const recordings = await ctx.db
      ...
      .order("desc")
      .collect();
    return Promise.all(recordings.map((r) => withUrl(ctx, r, true)));
  },
});
  ...
  handler: async (ctx, { surahNumber }) => {
    ...
    return Promise.all(
      recordings.map(async (r) => {
        const base = await withUrl(ctx, r, false);
        return {
          ...base,
...
// audioDataUrl is the compact fallback used when direct storage upload isn't
// available; storageId is the standard path (generateUploadUrl -> POST -> id).
async function withUrl(ctx: any, r: any, mine: boolean) {
  const url = r.storageId
    ? await ctx.storage.getUrl(r.storageId)
    : (r.audioDataUrl ?? undefined);
  const { audioDataUrl: _audio, ...rest } = r;
  return { ...rest, url, isMine: mine, userName: mine ? "You" : rest.userName };
}
  ...
  handler: async (ctx, args) => {
    ...
    const surah = args.surahNumber
      ? await ctx.db
          ...
          .unique()
      : undefined;
    const { audioDataUrl, ...rest } = args;
    return ctx.db.insert("recordings", {
      ...rest,
      ...(audioDataUrl && !rest.storageId ? { audioDataUrl } : {}),
      ownerId: user!._id,
